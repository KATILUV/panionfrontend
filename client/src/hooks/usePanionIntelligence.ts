/**
 * Panion Intelligence Hooks
 * Custom React hooks for interacting with Panion's enhanced intelligence capabilities
 */
...
export function useInsights(sessionId: string) {
  ...
  const { data, isLoading, error, refetch } = useQuery<{ insights: Insight[] }>({
    queryKey: ['/api/panion/intelligence/insights', sessionId],
    queryFn: async () => {
      const res = await apiRequest('GET', `/api/panion/intelligence/insights/${sessionId}`);
      return res.json();
    },
  ...
  const { mutate: generateInsights, isPending: isGenerating } = useMutation({
    mutationFn: async () => {
      const res = await apiRequest('POST', `/api/panion/intelligence/insights/${sessionId}/generate`);
      return res.json();
    },
    onSuccess: (newData) => {
      // Update cache with new insights
      queryClient.setQueryData(['/api/panion/intelligence/insights', sessionId], (oldData: any) => {
...
export function useReasoningPaths() {
  ...
  const generateReasoningPaths = async ({
    ...
    try {
      const res = await apiRequest('POST', '/api/panion/intelligence/reasoning-paths', {
        problem,
        sessionId,
        numPaths,
      });
...
export function useTaskDecomposition() {
  ...
  const decomposeTask = async ({
    ...
    try {
      const res = await apiRequest('POST', '/api/panion/intelligence/decompose-task', {
        taskDescription,
        sessionId,
      });
  ...
  const updateSubtaskStatus = async ({
    ...
    try {
      const res = await apiRequest('POST', '/api/panion/intelligence/update-subtask', {
        taskId,
        subtaskId,
        status,
      });
...
export function useVerification() {
  ...
  const verifyResult = async ({
    ...
    try {
      const res = await apiRequest('POST', '/api/panion/intelligence/verify', {
        originalQuery,
        result,
        sessionId,
      });
